package
{
	...
	/**
	 *
	 * 简单阴影就是在物体下方绘制一个 圆形的阴影区域
	 * 其实就是多渲染一个纹理, 这个纹理的 3D空间中的 X, Z坐标与物体一致, Y坐标自己适当调节
	 * 
	 * 未完成,因为不写框架的话, 两个物体很难支持！！
	 * 不过这个很容易就允许我偷懒下吧.
	 * 
	 *  
	 */	
	public class Test14SimpleShadow extends Sprite
	{
		[Embed(source="assets/images/metal04.png")]
		...
		private var needParallel:Boolean = false;
		private var needPoint:Boolean = false;
		...
		private var shadowList:Array = [];
		public function Test14SimpleShadow()
		{
			super();
			...
			//------------------light--init----------------------
			//没有位置, 也没有方向
			ambient.ambient.hex = 0xFFFFFFFF;
			
			//没有位置, 只有方向
			parallel.diffuse.hex = 0xFF0000FF;
			parallel.dir.setTo(-1, 0, 1);
			parallel.dir.normalize(parallel.dir);
			
			//点光源 没有方向, 只有位置
			point.diffuse.hex = 0xFF00FF00
...
class Utils
{
											...
											uva:Point, uvb:Point, uvc:Point, uvd:Point):void
	{
		...
		for(var x:int = sx; x < ex; x++) {
			...
			var textureC:uint = texture.getPixel(u,v);
			var red:uint = (interpolate(scr, ecr, gradient)/255 * (textureC >> 16 & 0xFF))>>0;
			var green:uint = (interpolate(scg, ecg, gradient)/255 * (textureC >> 8 & 0xFF)) >> 0;
			var blue:uint = (interpolate(scb, ecb, gradient)/255  * (textureC & 0xFF)) >> 0;
			if(red > 255) red = 255;
