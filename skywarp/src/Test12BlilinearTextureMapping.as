package
{
	...
	public class Test12BlilinearTextureMapping extends Sprite
	{
		...
		private var needAmbient:Boolean = true;
		private var needParallel:Boolean = true;
		private var needPoint:Boolean = true;
		...
		public function Test12BlilinearTextureMapping()
		{
			super();
			stage.addChild(bmp);
...
//------------------light--init----------------------
			//没有位置, 也没有方向
			ambient.ambient.hex = 0xFF880000;
			
			//没有位置, 只有方向
			parallel.diffuse.hex = 0xFF00FF00;
			parallel.dir.setTo(-1, 0, 1);
...
//------------------cube-----------------------------			
			...
			//2, 1, 0
			uvList[0] = [];
			uvList[0][0] = new Point(0, 0);
			uvList[0][1] = new Point(0, wall.height - 1);
			uvList[0][2] = new Point(wall.width - 1, wall.height - 1);
			//3, 2, 0
			uvList[1] = [];
			uvList[1][0] = new Point(wall.width - 1, 0);
			uvList[1][1] = new Point(0, 0);
			uvList[1][2] = new Point(wall.width - 1, wall.height - 1);
			//4, 7, 0
			uvList[2] = [];
			uvList[2][0] = new Point(0, wall.height - 1);
			uvList[2][1] = new Point(wall.width - 1, wall.height - 1);
			uvList[2][2] = new Point(0, 0);
			//7, 3, 0
			uvList[3] = [];
			uvList[3][0] = new Point(wall.width - 1, wall.height - 1);
			uvList[3][1] = new Point(wall.width - 1, 0);
			uvList[3][2] = new Point(0, 0);
			//6, 7, 4
			uvList[4] = [];
			uvList[4][0] = new Point(0, wall.height - 1);
			uvList[4][1] = new Point(wall.width - 1, wall.height - 1);
			uvList[4][2] = new Point(wall.width - 1, 0);
			//5, 6, 4
			uvList[5] = [];
			uvList[5][0] = new Point(0, 0);
			uvList[5][1] = new Point(0, wall.height - 1);
			uvList[5][2] = new Point(wall.width - 1, 0);
			//2, 6, 1
			uvList[6] = [];
			uvList[6][0] = new Point(0, 0);
			uvList[6][1] = new Point(0, wall.height - 1);
			uvList[6][2] = new Point(wall.width - 1, 0);
			//6, 5, 1
			uvList[7] = [];
			uvList[7][0] = new Point(0, wall.height - 1);
			uvList[7][1] = new Point(wall.width - 1, wall.height - 1);
			uvList[7][2] = new Point(wall.width - 1, 0);
			//7, 6, 3
			uvList[8] = [];
			uvList[8][0] = new Point(0, wall.height - 1);
			uvList[8][1] = new Point(wall.width - 1, wall.height - 1);
			uvList[8][2] = new Point(0, 0);
			//6, 2, 3
			uvList[9] = [];
			uvList[9][0] = new Point(wall.width - 1, wall.height - 1);
			uvList[9][1] = new Point(wall.width - 1, 0);
			uvList[9][2] = new Point(0, 0);
			//5, 4, 0
			uvList[10] = [];
			uvList[10][0] = new Point(0, wall.height - 1);
			uvList[10][1] = new Point(wall.width - 1, wall.height - 1);
			uvList[10][2] = new Point(wall.width - 1, 0);
			//1, 5, 0
			uvList[11] = [];
			uvList[11][0] = new Point(0, 0);
			uvList[11][1] = new Point(0, wall.height - 1);
			uvList[11][2] = new Point(wall.width - 1, 0);
...
//			Utils.drawTriangle(tempColor1, tempColor2, tempColor3, p1, p2, p3, uv1, uv2, uv3, bmd, wall.bitmapData);
		...
		private var randCount:uint = 50;
		private var rotaX:Number = Math.random() * 20 - 10;
		private var rotaY:Number = Math.random() * 20 - 10;
		private var rotaZ:Number = Math.random() * 20 - 10;
		protected function onEnterFrame(event:Event):void
		{
			if(randCount == 0){
				randCount = 50;
				rotaX = Math.random() * 20 - 10;
				rotaY = Math.random() * 20 - 10;
				rotaZ = Math.random() * 20 - 10;
				trace("rota change");
			}
			randCount--;
...
//						p3D.rotateZ(1);
		...
		private function fillTriangle():void
		{
			...
			for(var i:int = 0; i < this.polygonList.length - 2; i+=3)
			{
				...
				var uv1:Point = this.uvList[i/3 >>0][0];
				var uv2:Point = this.uvList[i/3 >>0][1];
				var uv3:Point = this.uvList[i/3 >>0][2];
				
				Utils.drawTriangle(c1, c2, c3, p1, p2, p3, uv1, uv2, uv3, bmd, wall.bitmapData, colorHex);
//				Utils.drawTriangle(tempColor1, tempColor2, tempColor3, p1, p2, p3, uv1, uv2, uv3, bmd, wall.bitmapData, colorHex);
			}
		}
...
//			var rl3:uint = r3, gl3:uint = g3, bl3:uint = b3;
			...
			if(needParallel)
			{
				...
				dp = parallel.dir.dot(n1.mult(-1, tempP1));
				if(dp > 0)
				{
					//这个光强跟夹角有关
					len = n1.magnitude();
				...
				dp = parallel.dir.dot(n2.mult(-1, tempP2));
				if(dp > 0)
				{
					//这个光强跟夹角有关
					len = n2.magnitude();
				...
				dp = parallel.dir.dot(n3.mult(-1, tempP3));
				if(dp > 0)
				{
					//这个光强跟夹角有关
					len = n3.magnitude();
			...
			if(needPoint)
			{
				...
				var light:SWPoint3D = point.pos.minus(p1World, tempP4);//记住这里也要归一化
				dp = light.dot(n1);
				if(dp > 0){
					len = n1.magnitude();
...
//					trace(cosT);
				...
				light = point.pos.minus(p2World, tempP4);
				dp = light.dot(n2);
				if(dp > 0){
					len = n2.magnitude();
					dist = light.magnitude();
...
//					trace(cosT);
				...
				light = point.pos.minus(p3World, tempP4);
				dp = light.dot(n3);
				if(dp > 0){
					len = n3.magnitude();
					dist = light.magnitude();
...
class Utils
{
											...
											uva:Point, uvb:Point, uvc:Point, uvd:Point):void
	{
		...
		var su:Number = interpolate(uva.x/pa.z , uvb.x/pb.z, gradient1);
		var eu:Number = interpolate(uvc.x/pc.z , uvd.x/pd.z, gradient2);
		
		var sv:Number = interpolate(uva.y/pa.z, uvb.y/pb.z, gradient1);
		var ev:Number = interpolate(uvc.y/pc.z, uvd.y/pd.z, gradient2);
		...
		for(var x:int = sx; x < ex; x++) {
			...
			var u:Number = interpolate(su, eu, gradient);
			var v:Number = interpolate(sv, ev, gradient);
			...
			var uInt:uint = u >> 0;
			var vInt:uint = v >> 0;
			var uFloat:Number = u - uInt;
			var vFloat:Number = v - vInt;
			var uOneMinus:Number = 1 - uFloat;
			var vOneMinus:Number = 1 - vFloat;
			
			
			var texture00:uint = texture.getPixel(uInt,vInt);
			var texture10:uint = texture.getPixel(uInt + 1,vInt);
			var texture01:uint = texture.getPixel(uInt,vInt + 1);
			var texture11:uint = texture.getPixel(uInt + 1,vInt + 1);
			
			var r00:uint = texture00 >> 16 & 0xFF;
			var g00:uint = texture00 >> 8 & 0xFF;
			var b00:uint = texture00 & 0xFF;

			var r10:uint = texture10 >> 16 & 0xFF;
			var g10:uint = texture10 >> 8 & 0xFF;
			var b10:uint = texture10 & 0xFF;
			
			var r01:uint = texture01 >> 16 & 0xFF;
			var g01:uint = texture01 >> 8 & 0xFF;
			var b01:uint = texture01 & 0xFF;
			
			var r11:uint = texture11 >> 16 & 0xFF;
			var g11:uint = texture11 >> 8 & 0xFF;
			var b11:uint = texture11 & 0xFF;
			
			var muti00:Number = uOneMinus * vOneMinus;
			var muti10:Number = uFloat * vOneMinus;
			var muti01:Number = uOneMinus * vFloat;
			var muti11:Number = uFloat * vFloat;
			
			var redT:uint =  (muti00 * r00 + muti10 * r10 + muti01 * r01 + muti11 * r11) >> 0;
			var greenT:uint =  (muti00 * g00 + muti10 * g10 + muti01 * g01 + muti11 * g11) >> 0;
			var blueT:uint =  (muti00 * b00 + muti10 * b10 + muti01 * b01 + muti11 * b11) >> 0;
			
			var red:uint = (interpolate(scr, ecr, gradient)/255 * redT)>>0;
			var green:uint = (interpolate(scg, ecg, gradient)/255 * greenT) >> 0;
			var blue:uint = (interpolate(scb, ecb, gradient)/255  * blueT) >> 0;
