package
{
	 ...
	 * 仿射纹理映射 和 透视纹理映射
	 * 
	 * 仿射纹理映射：纹理图不做任何变换（假设是正方形）, 假如你平面做过变换之后, 你还是在一个正方形的纹理上去做插值运算, 这很显然是不对的
	 * 因为投影屏幕的线并不是按照线性关系变化的
	 * 
	 * 透视纹理变换：需要在变换后的纹理上去取值, 才能取得正确的值
	 * 
	 * 这里先实现仿射纹理变换, 之后会有透视纹理变换demo
	 * 
	 *  
	 */	
	public class Test8GouraudShading extends Sprite
	{
		...
		public function Test8GouraudShading()
		{
			super();
			stage.addChild(bmp);
			var p:SWPoint3D;
...
//				p3D.rotateZ(1);
		...
		private function fillTriangle():void
		{
			...
			for(var i:int = 0; i < this.polygonList.length - 2; i+=3)
			{
				...
				Utils.drawTriangle(c1, c2, c3, p1, p2, p3, bmd, colorHex);
			}
		}
...
//				trace(tempP1, tempP2);
				...
				var n:SWPoint3D = tempP2.cross(tempP1, tempP3);
...
class Utils
{
	public static function drawTriangle(c1:Color, c2:Color, c3:Color, 
										...
										bmd:BitmapData, color:uint = 0):void
	{
...
//			trace("p2 at right of p1p3");
			for(y = (p1.y >> 0) ; y <= (p3.y >> 0); y++)
			{
				if(y < p2.y)
				{
					processScanLine(y, p1, p3, p1, p2, bmd, c1, c3, c1, c2);
				}else
				{
					processScanLine(y, p1, p3, p2, p3, bmd, c1, c3, c2, c3);//平顶 如果 p1.y == p2.y 直接就处理这里了, 精妙!
				}
			}
		}else
		{
//			trace("p2 at left of p1p3");
			for(y = (p1.y >> 0) ; y <= (p3.y >> 0); y++)
			{
				if(y < p2.y)
				{
					processScanLine(y, p1, p2, p1, p3, bmd, c1, c2, c1, c3);
				}else
				{
					processScanLine(y, p2, p3, p1, p3, bmd, c2, c3, c1, c3);
				}
			}
		}
	}
