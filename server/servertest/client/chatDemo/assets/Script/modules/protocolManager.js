...
function registerBufferEncode(sn, cn, func){
    ...
    if(!encodes[key]){
        ...
        cc.warn("encode function conflicted!", "service:", sn, "command:", cn);
    }
}
...
function registerBufferDecode(sn, cn, func){
    ...
    if(!decodes[key]){
        ...
        cc.warn("decode function conflicted!", "service:", sn, "command:", cn);
    }
}
...
function bufferEncode(sn, cn, body){
    ...
    if(!encodes[key]){
        cc.error("No such encode function!", "service:", sn, "command:", cn);
    }else{
...
function bufferDecode(buf){
    ...
    if(!decodes[key]){
        cc.error("No such decode function!", "service:", sn, "command:", cn);
    }else{
        cmd = decodes[key](buf);
    }
